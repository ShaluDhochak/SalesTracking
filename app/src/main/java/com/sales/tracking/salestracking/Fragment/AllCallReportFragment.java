...
import com.sales.tracking.salestracking.Adapter.VisitDoneManagerReportAdapter;
...
public class AllCallReportFragment extends Fragment {
    ...
    AllCallReportAdapter allCallReportAdapter;
    ...
    private void getAllSalesCallReportManagerRecyclerView(){
        if (Connectivity.isConnected(getActivity())) {
            ...
            GSONRequest<AllSalesCallReportBean> dashboardGsonRequest = new GSONRequest<AllSalesCallReportBean>(
                    Request.Method.POST,
                    Url,
                    AllSalesCallReportBean.class, map,
                    new com.android.volley.Response.Listener<AllSalesCallReportBean>() {
                        @Override
                        public void onResponse(AllSalesCallReportBean response) {
                            try{
                                if (response.getSp_all_calls().size()>0){
                                    saleCallReportHeader_rl.setVisibility(View.VISIBLE);


                                    allCallReportAdapter = new AllCallReportAdapter(getActivity(),response.getSp_all_calls(), AllCallReportFragment.this);
                                    ...
                                    viewSaleCallReport_rv.setAdapter(allCallReportAdapter);
    ...
    public void getAllSalesCallData(AllSalesCallReportBean.Sp_All_Calls bean){
